      ...
      <section className={styles.workspace}>
        ...
        <div className={styles.content}>
          ...
          <section className={styles.panel} aria-labelledby="booking-requests-title">
            ...
            {enquiries.length ? <div className={styles.reservationList}>{enquiries.map((item) => <article key={item.id}><div><strong>{item.fullName}</strong><small><a href={`tel:${item.phone}`}>{item.phone}</a> · <a href={`mailto:${item.email}`}>{item.email}</a></small></div><div><span>{item.checkIn} → {item.checkOut}</span><small>{item.guestCount} guest{item.guestCount === 1 ? "" : "s"} · {item.roomTypeName ?? "Best available room"}</small></div><b data-status={item.status}>{item.status}</b></article>)}</div> : <div className={styles.emptyState}><span aria-hidden="true">⌁</span><h3>No booking requests yet</h3><p>Guest enquiries submitted from the booking form will appear here.</p></div>}
          </section>
