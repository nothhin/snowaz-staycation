...
export function AdminMobileNav({classes}:{classes:MobileNavClasses}){
  ...
  return <><button className={classes.button} type="button" aria-label="Open admin navigation" aria-expanded={open} aria-controls="mobile-admin-navigation" onClick={()=>setOpen(true)}><span aria-hidden="true">☰</span></button>{open?<button className={classes.backdrop} type="button" aria-label="Close admin navigation" onClick={()=>setOpen(false)}/>:null}{open?<aside id="mobile-admin-navigation" className={`${classes.drawer} ${classes.drawerOpen}`}><div className={classes.drawerHeader}><div><strong>SnowAZ</strong><small>Property admin</small></div><button className={classes.closeButton} type="button" aria-label="Close admin navigation" onClick={()=>setOpen(false)}>×</button></div><nav aria-label="Mobile admin navigation">{navigation.map((item,index)=><a className={active===item.id?classes.active:undefined} href={item.href} key={item.id} onClick={()=>setOpen(false)}><span aria-hidden="true">{String(index+1).padStart(2,"0")}</span>{item.label}</a>)}</nav></aside>:null}</>;
}
