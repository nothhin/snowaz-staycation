...
export function BookingRequestsPanel({
  ...
  const requests = useMemo(
    () =>
      enquiries.filter(
        (item) => !["confirmed", "cancelled", "declined"].includes(item.status),
      ),
    [enquiries],
  );
  const filtered = useMemo(
    () =>
      requests.filter((item) => {
        const matchesQuery =
          !query ||
          [item.fullName, item.phone, item.email, item.depositReference].some(
  ...
  return (
    ...
    >
      <div className={styles.panelHeading}>
        <div>
          <p className={styles.eyebrow}>Guest website</p>
          <h2 id="booking-requests-title">Booking requests</h2>
        </div>
      ...
      <div className={styles.bookingFilters}>
        <label>
          ...
          <input
            ...
            placeholder="Guest, phone, email, or transfer reference"
          />
        </label>
        <label>
          <span>Show</span>
      ...
      {filtered.length ? (
        <div
          className={`${styles.reservationList} ${styles.bookingRequestList}`}
        >
          {filtered.map((item) => (
            <article key={item.id}>
              <div>
                ...
                <details className={styles.bookingDetails}>
                  ...
                  <dl>
                    ...
                    <div>
                      <dt>Deposit</dt>
                      <dd>{item.depositStatus.replaceAll("_", " ")}</dd>
                    </div>
                  </dl>
                </details>
              </div>
              <div>
                <span>{formatStayRange(item.checkIn, item.checkOut)}</span>
                <small>
                  {item.guestCount} guest{item.guestCount === 1 ? "" : "s"} ·{" "}
                  {item.guestCount >= 4 ? "2 bedrooms" : "1 bedroom"}
                </small>
                {item.depositReference ? (
                  <div className={styles.paymentReferenceCard}>
                    <small>Sender/account name</small>
                    <strong>{item.depositSenderName || "Not provided"}</strong>
                    <small>Transaction reference</small>
                    <strong>{item.depositReference}</strong>
                    {item.depositSubmittedAt
                      ? <small>Submitted {new Date(item.depositSubmittedAt).toLocaleString("en-PH", { dateStyle: "long", timeStyle: "short" })}</small>
                      : null}
                  </div>
                ) : null}
                {item.depositRefundReference ? (
                  <small>Refund: {item.depositRefundReference}</small>
                ) : null}
              </div>
              <div className={styles.depositColumn}>
                ...
                <span
                  ...
                  data-status={item.depositStatus}
                >
