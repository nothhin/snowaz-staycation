...
export function BookingRequestsPanel({ enquiries, canManage }: { enquiries: AdminEnquiry[]; canManage: boolean }) {
  ...
  return <section className={styles.panel} aria-labelledby="booking-requests-title">
    ...
    {filtered.length ? <div className={`${styles.reservationList} ${styles.bookingRequestList}`}>{filtered.map((item) => <article key={item.id}>
      <div><strong>{item.fullName}</strong><small><a href={`tel:${item.phone}`}>{item.phone}</a>{item.email ? <> · <a href={`mailto:${item.email}`}>{item.email}</a></> : null}</small><small>Preferred: {item.preferredContact}</small><details className={styles.bookingDetails}><summary>View booking details</summary><dl><div><dt>Booking ID</dt><dd>{item.id}</dd></div><div><dt>Stay</dt><dd>{item.checkIn} → {item.checkOut}</dd></div><div><dt>Guests</dt><dd>{item.guestCount}</dd></div><div><dt>Room</dt><dd>{item.roomTypeName ?? "Best available room"}</dd></div><div><dt>Deposit</dt><dd>{item.depositStatus.replaceAll("_", " ")}</dd></div></dl></details></div>
      <div><span>{item.checkIn} → {item.checkOut}</span><small>{item.guestCount} guest{item.guestCount === 1 ? "" : "s"} · {item.roomTypeName ?? "Best available room"}</small>{item.depositReference ? <small>Transfer: {item.depositReference} · {item.depositSenderName}{item.depositSubmittedAt ? ` · ${new Date(item.depositSubmittedAt).toLocaleString("en-PH")}` : ""}</small> : null}{item.depositRefundReference ? <small>Refund: {item.depositRefundReference}</small> : null}</div>
