...
export function BookingRequestsPanel({ enquiries, canManage }: { enquiries: AdminEnquiry[]; canManage: boolean }) {
  ...
  return <section id="booking-requests" className={styles.panel} aria-labelledby="booking-requests-title">
    ...
    {filtered.length ? <div className={`${styles.reservationList} ${styles.bookingRequestList}`}>{filtered.map((item) => <article key={item.id}>
      <div><strong>{item.fullName}</strong><small>{item.phone}{item.email ? <> · <a href={`mailto:${item.email}`}>{item.email}</a></> : null}</small><a className={styles.callGuestButton} href={`tel:${item.phone}`} aria-label={`Call ${item.fullName}`}>Call guest</a><details className={styles.bookingDetails}><summary>View booking details</summary><dl><div><dt>Booking ID</dt><dd>{item.id}</dd></div><div><dt>Stay</dt><dd>{formatStayRange(item.checkIn,item.checkOut)}</dd></div><div><dt>Guests</dt><dd>{item.guestCount}</dd></div><div><dt>Bedroom access</dt><dd>{item.guestCount>=4?"2 bedrooms":"1 bedroom"}</dd></div><div><dt>Deposit</dt><dd>{item.depositStatus.replaceAll("_", " ")}</dd></div></dl></details></div>
