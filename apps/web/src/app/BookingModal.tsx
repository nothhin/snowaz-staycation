    ...
    <div className="booking-modal" role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} ref={dialogRef}>
      ...
      {state.status === "success" ? <div className="booking-modal-success" role="status">
        ...
        <form action={action} className="booking-modal-form">
          ...
          <input type="hidden" name="idempotencyKey" value={idempotencyKey.current} /><input type="hidden" name="roomTypeId" value="" /><input type="hidden" name="preferredContact" value="phone" />
          ...
          <label><span>Number of guests (maximum 8)</span><input name="guests" type="number" min="1" max="8" defaultValue="2" required /><small>2 guests include 1 bedroom; book 4 or more guests for access to both bedrooms.</small></label>
          <label><span>Full name</span><input name="fullName" autoComplete="name" required /></label>
          <div className="booking-modal-grid"><label><span>Email address (optional)</span><input name="email" type="email" autoComplete="email" /></label><label><span>Contact number (required)</span><input name="phone" type="tel" autoComplete="tel" inputMode="tel" placeholder="09xx xxx xxxx" required /><small>SnowAZ will call this number about your request.</small></label></div>
