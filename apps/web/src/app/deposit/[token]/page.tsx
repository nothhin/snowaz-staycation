...
export default async function DepositPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ submitted?: string; error?: string; reference?: string }> }) {
  ...
  return <main className={styles.shell}><LiveRouteRefresh /><header><Link href="/">SnowAZ Staycation</Link><span>Live private booking status</span></header><article className={styles.card}>
    ...
    <div className={styles.booking}><strong>{request.fullName}</strong><span>{formatStayRange(request.checkIn, request.checkOut)} · {request.guestCount} guest{request.guestCount === 1 ? "" : "s"}</span></div>
    ...
    <footer id="cancellation-help"><p>This private link becomes read-only after payment and expires 30 days after checkout or a completed refund. Save your booking reference for future status checks.</p><MessengerReceiptLink className={styles.messengerAction} label="Request cancellation or refund in Messenger" message={`Hello SnowAZ! I am ${request.fullName}. I would like help cancelling my stay on ${formatStayDate(request.checkIn)} to ${formatStayDate(request.checkOut)}${request.depositStatus === "verified" || request.depositStatus === "refund_pending" ? " and requesting the return of my ₱1,000 security deposit" : ""}. Please confirm the next steps.`} /><div className={styles.helpActions}><a href={propertyProfile.messengerUrl} target="_blank" rel="noreferrer">Need help? Open Messenger</a><a href={`tel:${propertyProfile.phoneHref}`}>Call {propertyProfile.phoneDisplay}</a></div></footer>
  </article></main>;
}

function InvalidDepositLink({ token }: { token: string }) { return <main className={styles.shell}><article className={styles.card}><ForgetBookingIfMatches url={`/deposit/${token}`} /><p className={styles.eyebrow}>Private link unavailable</p><h1>This private link has expired.</h1><p>Use your saved booking reference and contact number to check the current status, or contact SnowAZ before sending money.</p><Link className={styles.helpLink} href="/booking-status">Check booking status</Link></article></main>; }
