...
export type BookingLookupState = { status:"idle"|"found"|"error"; message?:string; booking?:{ checkIn:string; checkOut:string; guests:number; status:string; depositStatus:string; depositExpiresAt:string|null } };
export async function lookupBooking(_previous: BookingLookupState, formData: FormData): Promise<BookingLookupState> {
  ...
  return { status:"found", booking:{ checkIn:row.check_in as string, checkOut:row.check_out as string, guests:row.guest_count as number, status:row.booking_status as string, depositStatus:row.deposit_status as string, depositExpiresAt:row.deposit_expires_at as string|null } };
}
