...
export type BookingActionState = { status: "idle" | "success" | "error"; message?: string; bookingReference?: string; depositLink?: string; depositExpiresAt?: string };
...
async function saveBookingRequest(formData: FormData) {
  ...
  try {
    const { data, error } = await supabase.rpc("submit_snowaz_booking_request", {
      ...
      requests: parsed.data.specialRequests || "",
    ...
    const result = { bookingReference: booking.booking_reference as string, depositExpiresAt: booking.deposit_expires_at as string };
    ...
    return { ok: false as const, message: "Those dates may no longer be available. Refresh the calendar or contact SnowAZ directly." };
  }
}
