...
export const stayDateSchema = z.string().refine(isCalendarDate, "Choose a valid calendar date.");
...
export function calculateStayTotalMinor(nightlyRateMinor: number, nights: number) {
  if (!Number.isSafeInteger(nightlyRateMinor) || nightlyRateMinor < 0) {
    throw new RangeError("Nightly rate must be a non-negative integer in minor units.");
  }
  if (!Number.isSafeInteger(nights) || nights < 1) {
    throw new RangeError("Stay length must be a positive whole number of nights.");
  }

  const total = nightlyRateMinor * nights;
  if (!Number.isSafeInteger(total)) throw new RangeError("Stay total exceeds the safe integer range.");
  return total;
}
