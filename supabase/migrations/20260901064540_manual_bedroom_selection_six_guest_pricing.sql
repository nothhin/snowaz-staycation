-- New bookings use guest-selected bedrooms and a six-guest maximum.
-- The table constraint remains compatible with historical four- and eight-guest bookings.
...
alter table public.booking_requests
  add constraint booking_requests_bedroom_capacity_valid check (
    ...
    or (bedroom_choice = 'bedroom_2' and guest_count between 2 and 4)
    or (bedroom_choice = 'both_bedrooms' and guest_count between 4 and 8)
  );
...
begin
  if char_length(trim(guest_name)) not between 2 and 120
    ...
    or not (
      (bedroom_selection='bedroom_1' and guests between 1 and 2)
  ...
  base_rate := case when bedroom_selection='both_bedrooms' then 240000 else 190000 end;
  extra_count := case when bedroom_selection='bedroom_2' then greatest(guests-2,0) when bedroom_selection='both_bedrooms' then greatest(guests-5,0) else 0 end;
...
revoke all on function public.submit_snowaz_booking_request(uuid,text,text,text,date,date,integer,text,text,text,text,text,text) from public,authenticated;
grant execute on function public.submit_snowaz_booking_request(uuid,text,text,text,date,date,integer,text,text,text,text,text,text) to anon;
...
declare
  ...
  nights integer; base_rate bigint; parking_rate bigint; extra_count integer; extra_charge bigint; parking_charge bigint;
begin
  ...
  where deposit_token_hash=token_hash and deposit_token_expires_at>now()
    and status in ('pending','contacted') and deposit_status in ('not_requested','awaiting_payment')
  for update;
...
returns boolean language plpgsql security definer set search_path='' as $$
declare
  nights integer; base_rate bigint; extra_count integer; extra_charge bigint; parking_rate bigint; parking_charge bigint; booking_total bigint; already_paid bigint;
  booking public.booking_requests%rowtype; strict_selection boolean; legacy_selection boolean;
begin
  ...
  strict_selection := guests between 1 and 6 and (
    (bedroom_selection='bedroom_1' and guests between 1 and 2)
    or (bedroom_selection='bedroom_2' and guests between 2 and 3)
    or (bedroom_selection='both_bedrooms' and guests between 4 and 6)
  );
  legacy_selection := guests=booking.guest_count and bedroom_selection=booking.bedroom_choice;
  if departure<=arrival or not (strict_selection or legacy_selection)
  ...
  nights:=departure-arrival;
  if strict_selection then
    base_rate:=case when bedroom_selection='both_bedrooms' then 240000 else 190000 end;
    extra_count:=case when bedroom_selection='bedroom_2' then greatest(guests-2,0) when bedroom_selection='both_bedrooms' then greatest(guests-5,0) else 0 end;
  else
    base_rate:=booking.base_nightly_rate_minor;
    extra_count:=booking.additional_guest_count;
  end if;
  extra_charge:=extra_count*30000*nights;
  parking_rate:=booking.parking_nightly_rate_minor;
  parking_charge:=parking_rate*nights;
  booking_total:=base_rate*nights+extra_charge+parking_charge;
  ...
  update public.snowaz_calendar_ranges set check_in=arrival,check_out=departure,display_status=case when booking.status='confirmed' then 'booked' else 'pending' end where source_kind='booking_request' and source_id=target_id;
  if not found then insert into public.snowaz_calendar_ranges(source_kind,source_id,check_in,check_out,display_status) values('booking_request',target_id,arrival,departure,case when booking.status='confirmed' then 'booked' else 'pending' end); end if;
